...
create_default_dir() {
       if [ ! -d "(" ]; then
           ...
           mkdir -p "("
           maybe_die "Couldn't create directory ("
           echo "- Changing permissions of '(' to 02755"
           chmod 02755 "("
           maybe_die "Couldn't change permissions for ("
                  if [ -n "$CELERYD_USER"  ]; then
                      echo "- Changing owner of '(' to '$CELERYD_USER'"
                      chown "$CELERYD_USER" "("
                      maybe_die "Couldn't change owner of ("
                             fi
                                    if [ -n "$CELERYD_GROUP"  ]; then
                                        echo "- Changing group of '(' to '$CELERYD_GROUP'"
                                        chgrp "$CELERYD_GROUP" "("
                                        maybe_die "Couldn't change group of ("
                                               fi
                                                  fi
                                              }
